<?php
	...
	class chat_stud extends ACore_student{
		...
		if(!$_GET['id_chat']){
			...
			if(!$id_chat){
				...
				/*$query_t = "SELECT c.id_chat, c.id_user_teacher, c.id_user_student,
					...
					JOIN users AS u
					JOIN chats AS c 
					ON c.id_chat = $id_chat AND c.id_chat = m.id_chat AND id_user_teacher = u.id AND id_user_student = $my_id
					ORDER BY m.date DESC";*/
				
				$query_t = "SELECT id, rights, last_name, first_name, middle_name 
					FROM users 
					WHERE id=$id_teacher";
				...
				for ($i = 0; $i < mysqli_num_rows($result_t); $i++){
					...
					//$id_teacher = $row_t['id_user_teacher'];
					$last_name = $row_t['last_name'];
					$first_name = $row_t['first_name'];
					$middle_name = $row_t['middle_name'];
				}
				

				//Данные о студенте
				$query_s = "SELECT c.id_chat, c.id_user_teacher, c.id_user_student,
						 m.id_msg, m.id_chat, m.send_to, m.text, m.msg_file, m.status_of_read, m.date, 
						 u.id, u.rights, u.last_name, u.first_name, u.middle_name 
					FROM messenges AS m 
					JOIN users AS u
					JOIN chats AS c 
					ON c.id_chat = $id_chat AND c.id_chat = m.id_chat AND id_user_student = u.id AND id_user_student = $my_id
					ORDER BY m.date DESC";
				...
				if($k == 0){
					...
					for ($i = 0; $i < mysqli_num_rows($result_s); $i++){
						...
						if($row['msg_file']==null){
							...
							<p>%s</p>
							<p><i>%s %s %s </i>|<i> %s</i></p></div>", $row['text'], $row_t['last_name'], $row_t['first_name'], $row_t['middle_name'], $row['date']);*/
							
							printf("<br><div style='background-color:#fff; border: 2px solid #cc0605; border-radius: 25px;box-sizing: border-box; padding: 20px;'>
							<p><div style='text-align:left'><b>%s %s %s</b>  | <i>%s</i> </div></p>
							<p><div style='text-align:left'>%s</div></p></div>", $row_t['last_name'], $row_t['first_name'], $row_t['middle_name'], $row['date'], $row['text']);
							}
							...
							<p>%s</p>
							<p><i>%s %s %s </i>|<i> %s</i></p></div>", $row['text'], $row['last_name'], $row['first_name'], $row['middle_name'], $row['date']);*/
							...
							if( $row['send_to'] == 'student' && $row['id_chat'] == $id_chat){
								/*printf("<br><div style='background-color:#fff; border: 2px solid #cc0605; border-radius: 25px;box-sizing: border-box; padding: 20px;'>
							<p>%s</p>
							<b><a style='color:#cc0605' href='%s'> <p><img src='file/icons8-досье-20.png'> %s</p></a></b>
							<p><i>%s %s %s </i>|<i> %s</i></p></div>", $row['text'], $row['msg_file'], basename($row['msg_file']), $row['last_name'], $row['first_name'], $row['middle_name'], $row['date']);*/
							
							printf("<br><div style='background-color:#fff3ed; border: 2px solid #cc0605; border-radius: 25px;box-sizing: border-box; padding: 20px;'>
							<p><div style='text-align:left'><b>%s %s %s</b>  | <i>%s</i> </div></p>
							<p><div style='text-align:left'>%s</div></p>
							
							<b><a style='color:#cc0605; text-align:left' href='%s'><p><img src='file/icons8-досье-20.png'> %s</p></a></b></div>", $row_t['last_name'], $row_t['first_name'], $row_t['middle_name'], $row['date'], $row['text'], $row['msg_file'], basename($row['msg_file']));
							}
							
							else if( $row['send_to'] == 'teacher' && $row['id_chat'] == $id_chat){
								/*printf("<br><div style='background-color:#fff3ed; border: 2px solid #cc0605; border-radius: 25px;box-sizing: border-box; padding: 20px;'>
							<div style='text-align:left'><p>%s</p></div>
							<b><a style='color:#cc0605' href='%s'> <p><img src='file/icons8-досье-20.png'> %s</p></a></b>
							<p><i>%s %s %s </i>|<i> %s</i></p></div>", $row['text'], $row['msg_file'], basename($row['msg_file']), $row['last_name'], $row['first_name'], $row['middle_name'], $row['date']);*/
