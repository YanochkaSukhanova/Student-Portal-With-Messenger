<?php
class notes_stud extends ACore_student {
	...
	public function get_content(){
		...
		echo "<br><div style='margin-right: 500px'>
			<a style='text-align:left' href='?option=add_note_stud'><button>Добавить заметку</button></a>
			</div>";
		...
		for ($i = 0; $i < mysqli_num_rows($result); $i++){
			...
			printf("<br><div style=' background-color:#fff3ed; border: 2px solid #cc0605; border-radius: 25px;box-sizing: border-box; padding: 20px;'>
				...
				<a style='color:#585858' href='?option=open_note_stud&id_text=%s'>Открыть</a> - 
				<a style='color:#585858' href='?option=update_note_stud&id_text=%s'>Изменить</a> - 
				<a style='color:red' href='?option=delete_note_stud&del_text=%s'>Удалить</a></p></div>",   $row['name'], $row['id_note'], $row['id_note'], $row['id_note']);
		}
		echo "</div>";
	}
}
