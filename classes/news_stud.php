...
class news_stud extends ACore_student {
	// Отображение всех новостей, доступных студенту в порядке "от новых к старым"
	public function get_content(){
		...
		for ($i = 0; $i < mysqli_num_rows($result); $i++){
			...
			if ($row['file_src'] == ""){
				...
				<p><div style='text-align:left'><b>Автор:</b> %s %s %s</div></p></div>", $row['name'], $row['text'], $row1['last_name'], $row1['first_name'], $row1['middle_name']);
			}
			else{
				printf("<br><div style='background-color:#fff; border: 2px solid #cc0605; border-radius: 25px;box-sizing: border-box; padding: 20px;'>
				<p><div style='text-align:left; font-size: 20px;'><b>%s</b></div></p>
				...
				<p><div style='text-align:left'><b>Автор:</b> %s %s %s</div></p></div>", $row['name'], $row['text'], $row['file_src'], basename($row['file_src']), $row1['last_name'], $row1['first_name'], $row1['middle_name']);
			}
		}
		echo "</div>";
	}
