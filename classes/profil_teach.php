<?php
class profil_teach extends ACore_teacher{
	...
	public function get_content(){
		...
		echo "<br><div style='margin-right: 500px'>
			<a style='text-align:left;' href='?option=redactor_profil_teach'><button>Редактировать данные</button></a>
				</div>";
		if ($row['img_avatar'] == null){
			...
			<td>
				...
				</div>", $row['img_avatar'], $row['last_name'], $row['first_name'], $row['middle_name'], $row['email'], $row['login']);
		}
		echo '<br></div>';	
	}
}
